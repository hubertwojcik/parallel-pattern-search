...
LABELS = {
    "seq":        "Sequential",
    "goroutines": "Goroutines (OpenMP equiv.)",
    "mpi":        "MPI",
...
ax.set_title("Speedup vs. number of threads/processes\n(10 MB, 100 patterns)", fontsize=13)

for impl in [("goroutines", [1, 2, 4, 8]), ("mpi", [1, 2, 4])]:
    impl_name = impl[0]
    rows = sorted(filter_rows(workers_data, impl=impl_name), key=lambda r: r["workers"])
    t1 = rows[0]["throughput_gbs"]
    xs = [r["workers"] for r in rows]
    ys = [r["throughput_gbs"] / t1 for r in rows]
    ax.plot(xs, ys, marker="o", label=LABELS[impl_name], color=COLORS[impl_name], linewidth=2)

max_w = 8
ax.plot([1, max_w], [1, max_w], "--", color="gray", linewidth=1, label="Ideal")
...
ax.axhline(1.0, linestyle="--", color="gray", linewidth=1, label="Ideal")
ax.set_xlabel("Threads / processes")
...
ax.set_title("Peak throughput comparison\n(100 MB, 100 patterns)", fontsize=13)

configs = [("seq", 1), ("goroutines", 4), ("mpi", 4), ("pfac", 1)]
...
fig, ax = plt.subplots(figsize=(7, 5))
ax.set_title("Throughput vs. text size\n(100 patterns, 4 threads/processes)", fontsize=13)
...
fig, ax = plt.subplots(figsize=(7, 5))
ax.set_title("Throughput vs. pattern count\n(10 MB text, 4 threads/processes)", fontsize=13)
